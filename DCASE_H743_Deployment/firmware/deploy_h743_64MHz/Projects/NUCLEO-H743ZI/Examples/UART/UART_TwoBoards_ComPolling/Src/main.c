 ...
 *   - SystemClock_Config: HSI 64MHz, 无 PLL (SMPS 板安全, 不碰 PWR)
...
int main(void)
{
    ...
    /* 2. HAL 初始化 + 时钟 (HSI 64MHz, 官方例程已验证) */
    HAL_Init();
    SystemClock_Config();
    ...
    /* [诊断] 复位后直接发固定字节, 验证 UART 硬件链路 (绕过 printf/fputc) */
    {
        const char test[] = "HELLO-UART-TEST\r\n";
        HAL_UART_Transmit(&huart3, (uint8_t *)test, sizeof(test) - 1, 1000);
    }

    printf("\r\n=== DCASE Scene Classifier (deploy_h743_64MHz) ===\r\n");
    ...
    while (1)
    {
        ...
        int len = snprintf(buf, sizeof(buf),
            "RESULT class=%d,%s,conf=%.3f,dsp=%.1fms,inf=%.1fms,total=%.1fms\r\n",
            cls, CLASS_NAMES[cls], s_ai_output_float[cls],
            cycles_to_ms(t_dsp), cycles_to_ms(t_inf), cycles_to_ms(t_total));
 ...
 * HSI 64MHz, 无 PLL —— 官方例程已验证 (SMPS 板安全, 避免 480MHz 启动失败问题)
 * ================================================================ */
static void SystemClock_Config(void)
{
